...
public interface AgentCommentRepository extends JpaRepository<AgentComment, Long> {
    ...
    // Find all comments for an agent within a date range
    @Query("SELECT ac FROM AgentComment ac WHERE ac.agent = ?1 AND ac.commentDate BETWEEN ?2 AND ?3")
    List<AgentComment> findByAgentAndCommentDateBetween(User agent, LocalDate startDate, LocalDate endDate);
